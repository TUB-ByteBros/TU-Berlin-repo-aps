...
Canvas color_corners(Canvas c) {
    ...
    c = canvas_set_black(c, 0, 0);           // Bottom-left
    c = canvas_set_black(c, width - 1, 0);   // Bottom-right
    c = canvas_set_black(c, 0, height - 1);  // Top-left
    c = canvas_set_black(c, width - 1, height - 1);  // Top-right
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    ...
    for (int dy = 0; dy < height; dy++) {
        for (int dx = 0; dx < width; dx++) {
            int cur_x = x + dx;
            int cur_y = y - dy;  // Subtract dy because y increases upward

            // Check if the current point is within the canvas bounds
            if (cur_x >= 0 && cur_x < canvas_w && cur_y >= 0 && cur_y < canvas_h) {
                c = canvas_set_black(c, cur_x, cur_y);
            }
        }
    }
...
Canvas draw_rectangle_via_corners(Canvas c, int x0, int y0, int x1, int y1) {
    int width = x1 - x0 + 1;
    int height = y0 - y1 + 1;  // Since y0 is top and y1 is bottom

    return draw_rectangle(c, x0, y0, width, height);
}

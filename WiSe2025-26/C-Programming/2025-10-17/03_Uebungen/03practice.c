...
{
  // Augen
  c = canvas_set_black(c, 1, 4); // linkes Auge
  c = canvas_set_black(c, 7, 4); // rechtes Auge

  c = canvas_set_black(c, 1, 2); // linkes unteres Auge
  c = canvas_set_black(c, 7, 2); // rechtes unteres Auge

  // Mundlinie (mittig, y = 1, von x = 2 bis 6)
  for (int x = 2; x <= 6; ++x)
  {
    c = canvas_set_black(c, x, 1);
  }
  return c;
}
...
Canvas i_told_you_about_stairs(Canvas c, int step_width, int step_height)
{
  for (int y = 0; y < canvas_height(c); y++)
  {
    for (int x = 0; x < canvas_width(c); x++)
    {
      int step_number = x / step_width;
      if (y < (step_number + 1) * step_height)
      {
        c = canvas_set_black(c, x, y);
      }
    }

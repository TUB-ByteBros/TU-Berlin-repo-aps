...
folgenden Befehl aus: clang -std=c11 -g -Wall 07ex_test.c -o 07ex_test.o -lm &&
./07ex_test.o
*/
...
int32_t frumpleflabblefizzledipp( int32_t x ) {
  /* Wir müssen x unverändert behalten, daher eine Kopie anlegen. */
  int32_t fx = x;    /* Kopie von x */
  fizzledipp( &fx ); /* fx := fizzledipp(x) */

  /* flabble erwartet Zeiger auf die beiden Operanden und liefert int32_t
   * zurück. */
  int32_t fl = flabble( &x, &fx );

  /* frumple nimmt (int32_t x, int32_t *y, int32_t *z) und schreibt das Ergebnis
   * nach z. */
  int32_t y  = 27;
  int32_t z  = 0;
  frumple( fl, &y, &z );
...
struct PileOfPancakes_ {
    ...
    PileOfPancakes *further_layers; /* Weitere Schichten, oder `null` falls
                                       keine mehr folgen. */
};

/*
Aufgabe 5a:
Geben sie zurück, ob die _erste_ Schicht des gegebenen Pfannkuchenhaufens eine
Fruchtschicht ist.
*/
bool starts_with_fruit( PileOfPancakes p ) {
  /* Die erste (unterste) Schicht ist in `p.layer` gespeichert. */
  return p.layer == Fruit;
}

/*
Aufgabe 5b:
Geben sie zurück, ob die _letzte_ Schicht des gegebenen Pfannkuchenhaufens eine
Fruchtschicht ist.
*/
bool ends_with_fruit( PileOfPancakes p ) {
  /* Gehe rekursiv/iterativ durch die weiteren Schichten bis zur letzten
   * (further_layers == NULL). */
  PileOfPancakes *cur = &p; /* beginne bei der übergebenen (lokalen) Struktur */
  while ( cur->further_layers != NULL ) {
...
/*
Aufgabe 5c:
Geben Sie zurück, _wie viele_ Fruchtschichten der gegebene Pfannkuchenhaufen
enthält.
*/
